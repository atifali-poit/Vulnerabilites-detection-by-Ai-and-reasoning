...
def main():
    try:
        ...
        # Go up one level to the project root (first_tutorial/)
        project_root = os.path.abspath(os.path.join(current_dir, '..'))
        
        # Define paths using the same variable names as before
        ...
        # 2. Load Data from interim folder
        train_data = load_data(os.path.join(interim_path, 'train_processed.csv'))
        test_data = load_data(os.path.join(interim_path, 'test_processed.csv'))

        # 3. Apply the TF-IDF logic
